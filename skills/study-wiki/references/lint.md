...
8. **Coverage against weights.** Domains of **active** exams from the profile with zero notes, ranked by exam weight. An untouched 30% domain outranks every style finding in this list. Parked exams are not a coverage finding.
9. **Staleness.** `last_updated` far older than the newest source on the same topic, or older than a vendor change you know about.
10. **Size.** A content note over ~200 lines gets split by subtopic and cross-linked. The index, the logs, the overlap table, the roadmap and the question banks are registers and are exempt.
...
16. **Roadmap is the only calendar.** Dates, gates, parks and triggers appear in `wiki/_meta/roadmap.md` and are linked from elsewhere, not copied. A second dated session table is a lint finding: keep the roadmap, delete or collapse the copy.
17. **Overlap sort.** Rows in `wiki/_meta/overlap.md` still obey the sort key in that file. Parked exams have no `Next session` id.
